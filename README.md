...
A policy is just a prompt. The game server owns every decision: it composes
the acting seat's private cards, the public table state and that seat's
operator prompt, and asks Claude what the cog says and does with its chips.
...
src/cosino_player.nim       the player runnable: deliver a prompt, then idle
