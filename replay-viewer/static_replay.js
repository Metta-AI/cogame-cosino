// Cosino static replay shell: fetches the replay named by ?replay=<url>,
// hands the bytes to the wasm module (which re-derives the state timeline with
// the same Nim sim the game server runs), then drives the shared renderer with
// the resulting payload.
//
// The shell is the only thing on screen until the replay is in, so it has to
// be honest about waiting: the caption names what it is doing, a stalled fetch
// gives up after FETCH_TIMEOUT_MS instead of sitting on "LOADING" forever, and
// every failure offers a Retry that refetches without a page reload (the wasm
// module, once compiled, is reused).
(function () {
  "use strict";
  ...
  // Observatory episode page) can only see this document's `load` event, which
  // fires long before the wasm module has compiled and the replay has come
  // back from S3. So the shell tells the parent what it is doing: `loading` as
  // soon as this script runs, `ready` once the renderer has drawn its FIRST
  // FRAME, `error` when the replay cannot be shown. `ready` is posted from
  // attachReplay's onFirstFrame callback -- the same place that sets
  // data-replay-loaded="true" -- so the bridge and the attribute can never
  // disagree (chorus 3c11c953, 2026-08-24: a bare rAF pair beats the paint).
  // No secrets ride on it, so the target origin is "*".
  ...
  function fetchReplay(url) {
    ...
    // slow one, and the caption would say LOADING until the tab died.
  ...
  function start(module, bytes) {
    ...
    var ok = module._pkr_load_replay(ptr, bytes.length);
    module._free(ptr);
    if (!ok) {
      fail(readString(module, module._pkr_error_ptr(),
        module._pkr_error_len()) || "wasm rejected the replay");
      return;
    }
    var payload = JSON.parse(
      readString(module, module._pkr_payload_ptr(),
        module._pkr_payload_len())
    );
    var loading = document.getElementById("loading");
    if (loading) loading.style.display = "none";
    document.documentElement.removeAttribute("data-replay-error");
    CosinoRenderer.attachReplay({
      ...
      rungchip: document.getElementById("rungchip"),
  ...
  function load() {
    ...
    caption(attempt > 1 ? "RETRYING REPLAY… (attempt " + attempt + ")" :
      "LOADING REPLAY…");
    if (!modulePromise) {
      // MODULARIZE=1 + EXPORT_NAME=CosinoReplayModule: the emitted glue exposes
      // a FACTORY, and the shell must call it. Splicing one starter's shell
      // onto another's link flags is what deadlocked cogame-lantern.
      modulePromise = CosinoReplayModule().catch(function (error) {
